        ...
        <div class="h-screen grid place-items-center" v-else>
          ...
          >
            <div class="px-4 py-5 sm:p-6">
              ...
              <UForm :schema="emailShema" :state="state" class="space-y-4 mt-2" @submit="onSubmit">
                <UFormField label="Email" name="email" help="Type in your company email, an API key will be sent there">
                  <UInput v-model="state.email" class="w-full" />
                </UFormField>
                <UButton type="submit" block :loading="loading" loading-icon="line-md:loading-loop"> Submit </UButton>
              </UForm>
            </div>
          </div>
        </div>
